...
func NewHealthJob(repo *repositories.TunnelRepository) *HealthJob {
	return &HealthJob{
		repo: repo,
	}
}
...
func (h *HealthJob) StartHealthCheck() {
	go func() {
		...
		for {
			resp, err := http.Get(variables.GetAddressURL())
			if err != nil {
				failCount++
				if isConnectionRefused(err) {
					...
					h.repo.UpdateWarnCount(variables.GetTunnelID())
				} else {
...
func (h *HealthJob) PingToServer() {
	req, err := http.NewRequest("HEAD", utils.GetUrl("ping", variables.GetTunnelID(), variables.GetSubdomainBool(), variables.GetServerURL()), nil)
	if err != nil {
		logger.Log("ERROR", "error during send healthcheck challenge", []logger.LogDetail{
			{Key: "error", Value: err.Error()},
		})
		h.repo.UpdateErrorCount(variables.GetTunnelID())
		return
	}
	...
	if resp.Header.Get("Tunnerse") == "healthcheck-conclued" {
		...
		h.repo.UpdateHealthcheckCount(variables.GetTunnelID())
	} else {
...
func CloseConnection() error {
	payload := map[string]string{"name": variables.GetTunnelID()}
	data, err := json.Marshal(payload)
	if err != nil {
		return fmt.Errorf("encode JSON: %w", err)
	}

	resp, err := http.Post(variables.GetTunnelHTTPURL()+"/close", "application/json", bytes.NewBuffer(data))
	if err != nil {
