...
func NewActionsRepository(db *Database) *ActionsRepository {
	return &ActionsRepository{DB: db}
}

// Create cria ou inicializa registros de Tunnel e Info
func (r *ActionsRepository) Create(tunnel *models.Tunnel, info *models.Info) error {
	...
	_, err = tx.Exec(`
		INSERT OR IGNORE INTO Info (ID, Pid, Requests, Healthchecks, Warns, Errors)
		VALUES (?, ?, ?, ?, ?, ?)`,
		info.ID, info.Pid, info.Requests, info.Healthchecks, info.Warns, info.Errors,
	)
	if err != nil {
		return err
	}
...
func (r *ActionsRepository) GetPID(tunnelID string) (int, error) {
	var pid int
...
func (r *ActionsRepository) GetTunnel(id string) (*models.Tunnel, error) {
	var t models.Tunnel
...
func (r *ActionsRepository) UpdateTunnelStatus(tunnelID string, active bool) error {
	_, err := r.DB.DB.Exec(`UPDATE Tunnel SET Active = ? WHERE ID = ?`, active, tunnelID)
	return err
}

func (r *ActionsRepository) UpdateRequestCount(id string) {
	go func() {
		tx, err := r.DB.DB.Begin()
		if err != nil {
			logger.Log("ERROR", "failed to begin transaction", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
			return
		}
		defer tx.Rollback()
		...
		if err != nil {
			logger.Log("ERROR", "failed to increment requests", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
			return
		}
		...
		if rowsAffected == 0 {
			...
			if err != nil {
				logger.Log("ERROR", "failed to insert new info record", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
				return
			}
		}

		if err := tx.Commit(); err != nil {
			logger.Log("ERROR", "failed to commit transaction", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
			return
		}
	}()
}

func (r *ActionsRepository) UpdateHealthcheckCount(id string) {
	go func() {
		tx, err := r.DB.DB.Begin()
		if err != nil {
			logger.Log("ERROR", "failed to begin transaction", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
		...
		if err != nil {
			logger.Log("ERROR", "failed to increment healthchecks", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
			return
		}
...
func (r *ActionsRepository) UpdateWarnCount(id string) {
	go func() {
		tx, err := r.DB.DB.Begin()
		if err != nil {
			logger.Log("ERROR", "failed to begin transaction", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
		...
		if err != nil {
			logger.Log("ERROR", "failed to increment warns", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
			return
		}
...
func (r *ActionsRepository) UpdateErrorCount(id string) {
	go func() {
		tx, err := r.DB.DB.Begin()
		if err != nil {
			logger.Log("ERROR", "failed to begin transaction", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
		...
		if err != nil {
			logger.Log("ERROR", "failed to increment errors", []logger.LogDetail{{Key: "error", Value: err.Error()}}, false)
			return
		}
...
func (r *ActionsRepository) ListTunnels() ([]*models.Tunnel, error) {
...
func (r *ActionsRepository) InfoTunnel(tunnelID string) (*struct {
	ID           string
	Port         int
	Url          string
	Domain       string
