...
func NewServerService(repo *database.ActionsRepository) *ServerService {
...
func (s *ServerService) RegisterTunnel() (string, bool, error) {
	...
	resp, err := http.Post(utils.GetUrl("register"), "application/json", bytes.NewBuffer(data))
	if err != nil {
...
func (s *ServerService) SendResponseToServer(data *models.ResponseData) error {
	...
	_, err = http.Post(utils.GetUrl("response"), "application/json", bytes.NewBuffer(jsonData))
	return err
}
...
func (s *ServerService) StartTunnelLoop() {
	...
	for {
		...
		if len(errorTimestamps) >= rateLimitCount {
			logger.Log("FATAL", "target server did not respond, the tunnel was closed", []logger.LogDetail{}, false)
		}
		...
		if err != nil {
			if err.Error() == "tunnel has closed by server" {
				logger.Log("FATAL", "tunnel has closed by server", []logger.LogDetail{}, false)
			}

			if err.Error() == "response-time-exceeded" {
				logger.Log("FATAL", "reponse time exceeded, the tunnel was closed", []logger.LogDetail{}, false)
			}
		...
		if err != nil {
			logger.Log("WARN", "failed to FOWARD REQUEST", []logger.LogDetail{}, false)
			continue
		}

		if os.Getenv("TUNNERSE_BG") == "1" {
			s.repo.UpdateRequestCount(config.GetTunnelID())
		}

		err = s.SendResponseToServer(respData)
		if err != nil {
			logger.Log("FATAL", "error during send response to server", []logger.LogDetail{}, false)
			continue
		}
	}
}
...
func (s *ServerService) FetchRequest() (*models.RequestData, error) {
	resp, err := http.Get(utils.GetUrl("fetch"))
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
